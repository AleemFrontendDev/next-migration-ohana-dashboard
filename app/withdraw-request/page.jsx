...
export function Withdraw() {
  ...
  const fetchPermissionsAndWithdraw = async () => {
    const response = await fetch(
      `${BASE_URL}/withdraw-request?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
    ...
    if (jsonData.success) {
      setSpinner(false);
      setWithdraw(jsonData.withdrawRequest.data);
      setTotalWithdraw(jsonData.withdrawRequest.total);
    }
    if (!user.role_id) {
      ...
      formatter: (cell, row) => {
        ...
        return permissions && permissions.view ? (
          <Link
            ...
            href={ROUTES.withdraw_request + "/" + row.id}
          >
            {html}
          </Link>
        ) : (
