...
export function PaymentGateway() {
  ...
  const fetchPaymentGateway = async () => {
    const response = await fetch(
      `${BASE_URL}/payment-gateways?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
    ...
    if (jsonData.success) {
      setSpinner(false);
      setPaymentGateway(jsonData.paymentGateways.data);
      setTotalPaymentGateway(jsonData.paymentGateways.total);
    }
  ...
  const statusChange = async (check, row) => {
    ...
    if (permissions.edit) {
      // const paymentGatewayCol = doc(db.db, 'app_settings', 'payment_gateway', 'settings', row.payment_gateway_id);
      // const paymentGatewayData = await getDoc(paymentGatewayCol);
      // const getpaymentGatewayData = paymentGatewayData.data();
      // getpaymentGatewayData.activate = check;
      // await updateDoc(paymentGatewayCol, getpaymentGatewayData);
      const response = await fetch(
        `${BASE_URL}/payment-gateway-status-change`,
        {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
  ...
  const columns = [
    ...
    },
    {
      ...
      // sort: true,
      formatter: (cell, row) =>
        cell.map((country) => country.nicename).join(", "),
    },
    {
    ...
    },
    {
      ...
      formatter: (cell, row) => {
        ...
        return permissions && permissions.edit ? (
          <Link
            className="nav-link"
            href={ROUTES.payment_gateway + "/" + row.id + "/edit-payment-gateway"}
          >
            {html}
          </Link>
        ) : (
                              ...
                              <div className="col-xl-3 col-lg-6 col-sm-6 grid-margin-xl-0 grid-margin pl-3 pr-0 ml-auto text-right">
                                <Button variant="primary">
                                  <Link
                                    ...
                                    href={ROUTES.add_payment_gateway}
                                  >
                                    <Trans>Add New Payment Gateway</Trans>
                                  </Link>
                                </Button>
                              </div>
                            ) : (
