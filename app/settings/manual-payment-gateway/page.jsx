...
export function PaymentGateway() {
  ...
  const fetchPermissionsAndPaymentGateway = async () => {
    const response = await fetch(
      `${BASE_URL}/manual-payment-gateways?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
    ...
    if (jsonData.success) {
      setSpinner(false);
      setPaymentGateway(jsonData.manualPaymentGateways.data);
      setTotalPaymentGateway(jsonData.manualPaymentGateways.total);
    }
  ...
  useEffect(() => {
    if (!authToken) return;
    fetchPermissionsAndPaymentGateway();
  }, [authToken, country, page, sizePerPage, search, field, order, filter]);

  useInterval(fetchPermissionsAndPaymentGateway, 20000, 5);
  ...
  const statusChange = async (check, row) => {
    // Update PaymentGateway Status.
    if (permissions.edit) {
      // const manualPaymentGatewayCol = doc(db.db, 'app_settings', 'manual_payment_gateway', 'settings', row.payment_gateway_id);
      // const manualPaymentGatewayData = await getDoc(manualPaymentGatewayCol);
      // const getManualPaymentGatewayData = manualPaymentGatewayData.data();
      // getManualPaymentGatewayData.activate = check;
      // await updateDoc(manualPaymentGatewayCol, getManualPaymentGatewayData);
      const response = await fetch(
        `${BASE_URL}/manual-payment-gateway-status-change`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const columns = [
    ...
    {
      dataField: "mobile_name_1",
      text: "Name 1",
      sort: true,
    },
    {
      dataField: "mobile_number_1",
      text: "Number 1",
      sort: true,
    },
    {
      dataField: "mobile_name_2",
      text: "Name 2",
      sort: true,
    },
    {
      dataField: "mobile_number_2",
      text: "Number 2",
      sort: true,
    },
    {
    ...
    {
      dataField: "deposit_processing_fee",
      text: "Processing(Deposit)",
      sort: true,
      formatter: priceFormatter,
    },
    {
      dataField: "withdraw_processing_fee",
      text: "Processing(Withdrawal)",
      sort: true,
      formatter: priceFormatter,
    },
    {
      dataField: "deposit_service_fee",
      text: "Service(Deposit)",
      sort: true,
      formatter: priceFormatter,
    },
    {
      dataField: "withdraw_service_fee",
      text: "Service(Withdrawal)",
      sort: true,
      formatter: priceFormatter,
    },
    {
                              ...
                              <div className="col-xl-3 col-lg-6 col-sm-6 grid-margin-xl-0 grid-margin pl-3 pr-0 ml-auto text-right">
                                <Button variant="primary">
                                  <Link
                                    ...
                                    href={ROUTES.add_manual_payment_gateway}
                                  >
                                    <Trans>Add Manual Payment Gateway</Trans>
                                  </Link>
                                </Button>
                              </div>
                            ) : (
