...
export function Countries() {
  ...
  const fetchCountries = async () => {
    const response = await fetch(
      `${BASE_URL}/countries?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
    ...
    if (jsonData.success) {
      setSpinner(false);
      setCountries(jsonData.countries.data);
      setTotalCountries(jsonData.countries.total);
    }
    if (user || !user?.role_id) {
      setPermissions({ add: true, view: true, edit: true, delete: true });
    } else {
      var rolePermission = user.role.permissions.filter(
        (permission) => permission.section === "Countries"
      );
  ...
  const operatingChange = async (check, row) => {
    // Update Currency Status.
    if (permissions.edit) {
      const countryCol = doc(db.db, "countries", row.nicename);
      const countryData = await getDoc(countryCol);
      const getCountryData = countryData.data();
      getCountryData.operating_country = check;
      await updateDoc(countryCol, getCountryData);
      const response = await fetch(`${BASE_URL}/country-operating-change`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
  ...
  const columns = [
    {
      dataField: "flag",
      text: "Image",
      formatter: (cell, row) => <img className="w-[40px] h-[26px]" src={cell} />,
    },
    {
      dataField: "nicename",
      text: "Name",
    ...
    },
    {
      ...
      text: "Assign Currencies",
      // sort: true,
      formatter: (cell, row) =>
        cell.map((currency) => currency.code).join(", "),
    },
    {
      ...
      formatter: (cell, row) => {
        ...
        return permissions && permissions.edit ? (
          <Link
            ...
            href={ROUTES.countries + "/" + row.nicename + "/edit"}
          >
            {html}
          </Link>
        ) : (
                              ...
                              permissions.add ? (
                                <Button variant="primary">
                                  <Link
                                    ...
                                    href={ROUTES.assign_currencies_to_country}
                                  >
                                    <Trans>Assign Currencies To Country</Trans>
                                  </Link>
                                </Button>
                              ) : (
                                ""
