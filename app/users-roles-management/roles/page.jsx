...
export function Roles() {
    ...
    const fetchRoles = async () => {
        const response = await fetch(`${BASE_URL}/roles?page=${page}&size=${sizePerPage}`, {
            method: 'POST',
            headers: {
        ...
        if (jsonData.success) {
            setSpinner(false);
            setRoles(jsonData.roles.data);
            setTotalRoles(jsonData.roles.total);
        }
        if (user || !user?.role_id) {
            setPermissions({add: true, view: true, edit: true, delete: true});
        } else {
            var rolePermission = user.role.permissions.filter((permission) => permission.section === 'Roles');
    ...
    const columns = [
        ...
        },
        {
            ...
            text: 'Permissions',
            formatter: (cell, row) => {
                const html = <div className="icon-holder"><i className='mdi mdi-eye bg-secondary p-2 rounded'></i></div>;
                return permissions && permissions.view ?
                    <Link className="nav-link pl-0" href={ ROUTES.roles + "/" + row.id + ROUTES.permissions }>{ html }</Link>
                : html
            }
        },
    ];
                                                        ...
                                                        <div className="card">
                                                            <div className="card-body">
                                                                <div className="d-flex align-items-center mb-3">
                                                                    ...
                                                                    <div>
                                                                        <h6 className="font-weight-semibold mb-0 text-warning">Roles</h6>
                                                                    </div>
                                                                    {
                                                                        permissions && permissions.add ?
                                                                            <div className='col-xl-3 col-lg-6 col-sm-6 grid-margin-xl-0 grid-margin pl-3 pr-0 ml-auto text-right'>
                                                                                <Button variant="primary">
                                                                                    <Link className="text-white text-decoration-none" href={ ROUTES.add_new_role }>
