...
import { doc, getDoc, namedQuery } from "firebase/firestore";
...
const CustomModal = ({ show, onClose, onSave, updatedName, setUpdatedName, type }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={onClose}>
      <div className="bg-[#f3f3f3] border-2 border-[#fb2!important] py-6 px-[15px] rounded-[1rem] shadow-lg w-[500px] h-[245px]"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-lg font-semibold mb-4">Ohana and Manual Pools</h2>
        ...
        />
        <div className="mt-5 pt-3 flex justify-end items-center space-x-2 border-t-2 border-[#dee2e6]">
          <button onClick={onClose} className="px-4 py-2 bg-gray-300 rounded">Cancel</button>
          <button onClick={onSave} className="px-4 py-2 bg-[#fb2] text-white rounded">Save</button>
        </div>
      </div>
    </div>
  );
...
function GroupDetail() {
  ...
  const [extraFoundAmount, setExtraFoundAmount] = useState(0);
  ...
  const handleSave = async () => {
    try {
      await addititletochangePools(); // Ensure API call is completed
      handleClose();
    } catch (error) {
  ...
  const fetchGroupDetailAndPermissions = async () => {
    try {
      ...
      if (jsonData.status === "success") {
        const { data } = jsonData;
        setSpinner(false);
        setTotalPools(data.total_pools);
        setTotalMembers(data.members);
        setDepositAmount(parseFloat(data.totalDepositGroup));
        setWithdrawAmount(parseFloat(data.totalWithdrawalGroup));
        setExpensesAmount(parseFloat(data.expense));
        setExtraFoundAmount(parseFloat(data.extra_funds));
        setDisbursementAmount(parseFloat(data.disbursement));
        setGroupData({
          id: data.group_id,
      ...
      if (user || !user?.role_id) {
        ...
        var groupDetailPermission = user.role.permissions.filter(
          (permission) => permission.section === "Organization Group"
        );
        if (groupDetailPermission.length && groupDetailPermission[0].status) {
          setPermissions(...groupDetailPermission);
        }
        setAuthUser(user);
      }
    } catch (error) {
  ...
  const addititletochangePools = async () => {
    try {
      ...
      if (jsonData.status === "success") {
        const { data } = jsonData;
        setUpdatedName(data.title);
        setPools((prevPools) =>
          prevPools.map((pool) =>
            pool.id === idpool ? { ...pool, title: data.title } : pool
          )
        );
      }
    } catch (error) {
                ...
                <div className="card-body">
                  <div className="d-flex justify-content-between">
                    <div>
                      ...
                      <h4 className="mt-3">
                        <Trans>Group</Trans>: {groupData.name}
                      </h4>
                    </div>
                  </div>
                </div>
                ...
                <div className="card-body">
                  <div className="d-flex align-items-center mb-2">
                    ...
                    <div>
                      ...
                      <span className="d-flex align-items-end ">
                        <h6 className="font-weight-semibold mb-0">
                          {depositAmount.toFixed(2)} {groupData.currency_code}
                        </h6>
                      </span>
                    </div>
                  </div>
                ...
                <div className="card-body">
                  <div className="d-flex align-items-center mb-2">
                    ...
                    <div>
                      ...
                      <span className="d-flex align-items-end ">
                        <h6 className="font-weight-semibold mb-0">
                          {withdrawAmount.toFixed(2)} {groupData.currency_code}
                        </h6>
                      </span>
                    </div>
                  </div>
                ...
                <div className="card-body">
                  <div className="d-flex align-items-center mb-2">
                    ...
                    <div>
                      ...
                      <span className="d-flex align-items-end ">
                        <h6 className="font-weight-semibold mb-0">
                          {expensesAmount.toFixed(2)} {groupData.currency_code}
                        </h6>
                      </span>
                    </div>
                  </div>
              ...
              <div className="card h-100">
                <div className="card-body">
                  <div className="d-flex align-items-center mb-2">
                    ...
                    <div>
                      <h6 className="font-weight-semibold small mb-0 text-warning">
                        <Trans>Extra Fund</Trans>
                      </h6>
                      <span className="d-flex align-items-end ">
                        <h6 className="font-weight-semibold mb-0">
                          {extraFoundAmount?.toFixed(2)}{" "}
                          {groupData.currency_code}
                        </h6>
                      </span>
                    </div>
                  </div>
                        ...
                        >
                          ...
                          <td>
                            <div className="d-flex align-items-center">
                              ...
                              <div
                                ...
                                className="nav-link ml-2 cursor-pointer p-0"
                              >
                          ...
                          {/* <td>
                            ...
                            <> 
                              ...
                              <div onClick={() => handleShow(item.title, item.org_pool_type, item.id)}
                              className="nav-link"
                            >
                              <i className="mdi mdi-square-edit-outline"></i>
                            </div>
                          ...
                          <td>
                            {item.org_pool_type === "Manual Pool"
                              ? item.amount
                              : item.balance}
                          </td>
                          <td>
                            {
                              <Link
                                href={ROUTES.pool_contributions.replace(
                                  ":id",
                                  item.id
                                )}
                              >
