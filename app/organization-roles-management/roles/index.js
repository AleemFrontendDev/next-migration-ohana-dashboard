...
export function Roles() {
  ...
  console.log("organization 31", organizations);
  ...
  const fetchRoles = async () => {
    const response = await fetch(
      `${BASE_URL}/get-roles-by-org-id?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({ org_id: organizations, search, field, order }),
      }
    );
    const jsonData = await response?.json();
    console.log("get-roles-by-org-id jsonData", jsonData);
    if (jsonData.status === "success") {
      setSpinner(false);
      setRoles(jsonData?.data);
      setTotalRoles(jsonData?.data?.length);
    } else {
      console.log("Error fetching in get-roles-by-org-id data:", jsonData);
    }
    ...
    if (!user?.role_id) {
      ...
      var rolePermission = user?.role?.permissions.filter(
        (permission) => permission?.section === "Organization Roles"
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      text: <Trans>Permissions</Trans>,
      formatter: (cell, row) => {
        console.log("row", row);
        const html = (
          <div className="icon-holder">
            <i className="mdi mdi-eye bg-secondary p-2 rounded"></i>
          </div>
        );
        return (
          <Link className="nav-link pl-0" to={"/org-permissions/" + row.id}>
            {html}
          </Link>
        );
        // ROUTES.roles + "/" + row.id + ROUTES.permissions
