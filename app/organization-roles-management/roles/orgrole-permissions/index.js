...
export function RolePermissions() {
  ...
  const fetchPermissions = async () => {
    const response = await fetch(`${BASE_URL}/get-org-permission-by-role`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ role_id: id, search, field, order }),
    });
    const jsonData = await response.json();
    console.log("get-org-permission-by-role response jsonData", jsonData);
    if (jsonData.status === "success") {
      setPermission(jsonData.data);
      setTotalPermissions(jsonData.data.length);
    } else {
      console.log("error get-org-permission-by-role", jsonData);
    }
    // setSpinner(false);
    // setPermissions(jsonData.permissions.data);
    // setTotalPermissions(jsonData.permissions.total);
    // if (!jsonData.role) {
    ...
    if (!user.role_id) {
      setPermission({ add: true, view: true, edit: true, delete: true });
    } else {
  ...
  const statusChange = async (check, row) => {
    // Update Permissions.
    if (permission.edit) {
      const response = await fetch(`${BASE_URL}/permission-status-change`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${authToken}`, 
        ...
        body: JSON.stringify({ id: row, check }),
      });
      const jsonData = await response.json();
      if (jsonData.success) {
        fetchPermissions();
    ...
    },
    {
      ...
      formatter: (cell, row) => (
        <span
          ...
          style={permission.edit ? { cursor: "pointer" } : {}}
          onClick={() => statusChange("add", row.id)}
        >
          {cell ? "Yes" : "No"}
        </span>
      ),
    },
    {
      ...
      formatter: (cell, row) => (
        <span
          ...
          style={permission.edit ? { cursor: "pointer" } : {}}
          onClick={() => statusChange("view", row.id)}
        >
          {cell ? "Yes" : "No"}
        </span>
      ),
    },
    {
      ...
      formatter: (cell, row) => (
        <span
          ...
          style={permission.edit ? { cursor: "pointer" } : {}}
          onClick={() => statusChange("edit", row.id)}
        >
          {cell ? "Yes" : "No"}
        </span>
      ),
    },
    {
      ...
      formatter: (cell, row) => (
        <span
          ...
          style={permission.edit ? { cursor: "pointer" } : {}}
          onClick={() => statusChange("delete", row.id)}
        >
          {cell ? "Yes" : "No"}
        </span>
      ),
                              ...
                              <div className="col-xl-3 col-lg-6 col-sm-6 grid-margin-xl-0 grid-margin pl-3 pr-0 ml-auto text-right">
                                <Button variant="primary">
                                  <Link
                                    ...
                                    to={"/add-new-orgpermission/" + id}
                                  >
                                    <Trans>Add New Permission</Trans>
                                  </Link>
                                </Button>
                              </div>

                              //  /group-admin/edit-group/" + row.id
                            }
                            {/* <Button variant="primary">
<Link className="text-white text-decoration-none" to={ ROUTES.roles + "/" + id + ROUTES.permissions + ROUTES.add_new_permission }>
    <Trans>Add New Permission</Trans>
</Link>
</Button> */}
                          </div>
