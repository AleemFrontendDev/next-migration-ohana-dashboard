...
export function Users() {
  ...
  console.log("21 organizations", organizations);

  const fetchUsersAndPermission = async (org_id) => {
    const response = await fetch(
      `${BASE_URL}/get-org-admins?page=${page}&size=${sizePerPage}`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ org_id: org_id, search, field, order }),
      }
    );

    const jsonData = await response.json();
    console.log("get-org-admins api jsonDta", jsonData);
  ...
  // useEffect(() => {
  //   fetchUsersAndPermission();
  // }, [organizations, page, sizePerPage, search, field, order]);

  // useInterval(fetchUsers, 20000, 5);

  const fetchOrgDataByUser = async () => {
    try {
      ...
      await fetchUsersAndPermission(data.org_id);
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cell, row) => {
        return (
          <Link
            className="nav-link icon-holder bg-secondary text-center p-2 rounded"
            to={ROUTES.orgcustomer_id.replace(":id", row.appuser_id)}
          >
    ...
    },
    {
      ...
      formatter: (cell, row) => {
        return (
          <Link
            className="nav-link icon-holder bg-secondary text-center p-2 rounded"
            to={ROUTES.edit_org_user.replace(":id", row.appuser_id)}
          >
  ...
  //   // Update User Status.
  //   if (permissions.edit) {
  //     const userCol = doc(db.db, "users", row.user_id);
  //     const userData = await getDoc(userCol);
  //     const getUserData = userData.data();
  //     getUserData.account_rejected = !check;
  //     await updateDoc(userCol, getUserData);
